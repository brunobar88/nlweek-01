...
const Points = () => {
    ...
    useEffect(() => {
        async function laodPosition() {
            const { status } = await Location.requestPermissionsAsync();

            if(status !== 'granted') {
                Alert.alert("Ooops...", "Presisamos de sua localização para te mostrar pontos de coleta no mapa");
                return;
            }
            ...
            <View style={styles.container}>
                ...
                <View style={styles.mapContainer}>
                    {initialPosition[0] !== 0 && (
                        ...
                        {points.map(point => (
                            <Marker key={String(point.id)} onPress={() => handleNavigateDetail(point.id)} style={styles.mapMarker} 
